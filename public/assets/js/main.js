(function (window, document, $, undefined) {
    ...
    let sstJs = {
        ...
        methods: function (e) {
            ...
            sstJs.headerSticky();
            sstJs.counterUp();
            sstJs.productHoverTilt();
            sstJs.masonryActivation();
            sstJs.lightboxActivation();
            sstJs.boxHover();
            sstJs.boxHoverSlider();
            sstJs.pricingPlan();
            sstJs.wowActivation();
            sstJs.qtyBtn();
            sstJs.accordionBtnTwo();
            sstJs.bannerAnimation();
            sstJs.popupMobileMenu();
            sstJs.searchPopup();
            sstJs.bgMarque();
            sstJs.filterClickButton();
            sstJs.contactForm();
        },
        ...
        slickSlider: function () {
            ...
            $('.sst-home-five-testimonial-activation').slick({
                dots: true,
                infinite: true,
                arrows: false,
                speed: 500,
            ...
            $('.sst-course-details-related-course-carousel').slick({
                dots: false,
                infinite: true,
                arrows: false,
                speed: 1000,
            ...
            $('.sst-course-carousel-page-with-dots').slick({
                dots: true,
                infinite: true,
                arrows: false,
                speed: 1000,
            ...
            $('.sst-event-one-carousel-wrapper').slick({
                dots: true,
                infinite: true,
                arrows: false,
                speed: 1000,
        ...
        productHoverTilt: function () {
            if ( $.isFunction( $.fn.tilt ) ) {
                $( '.sst-single-product-thumb-wrapper' ).tilt( {
                    maxTilt: 30,
                    perspective: 1000,
        ...
        boxHover: function(){
            jQuery( '.edu-instructor-3-visible, .sst-hover-item-visible' ).on( 'mouseenter', function(){
                let selector = jQuery(this).parent().parent();
                selector.find( '.edu-instructor-3-visible, .sst-hover-item-visible' ).removeClass( 'sst-hover-active' );
                jQuery(this).addClass( 'sst-hover-active' );
            });
        },
        ...
        boxHoverSlider: function(){
            jQuery( '.edu-instructor-3-slider-visible' ).on( 'mouseenter', function(){
                ...
                selector.find( '.edu-instructor-3-slider-visible' ).removeClass( 'sst-hover-active' );
                jQuery(this).addClass( 'sst-hover-active' );
            });
        },
        ...
        searchPopup: function () {
            ...
            $('.edu-search-popup .sst-search-popup-field').on('click', function (e) {
                e.stopPropagation();
            })
        },
...
var $addNewItem = $('<a href="#" class="btn btn-sst-info">Add new item</a>');
...
var $collectionHolderp;
// the link which we click on to add new items
var $addNewItemp = $('<a href="#" class="btn btn-sst-info">Add new item</a>');
// when the page is loaded and ready
$(document).ready(function () {
    // get the collectionHolder, initilize the var by getting the list;
    $collectionHolderp = $('#prog_list');
    // append the add new item link to the collectionHolder
    $collectionHolderp.append($addNewItemp);
    // add an index property to the collectionHolder which helps track the count of forms we have in the list
    $collectionHolderp.data('index', $collectionHolderp.find('.panel').length)
    // finds all the panels in the list and foreach one of them we add a remove button to it
    // add remove button to existing items
    $collectionHolderp.find('.panel').each(function () {
        // $(this) means the current panel that we are at
        // which means we pass the panel to the addRemoveButton function
        // inside the function we create a footer and remove link and append them to the panel
        // more informations in the function inside
        addRemoveButtonp($(this));
    });
    // handle the click event for addNewItem
    $addNewItemp.click(function (e) {
        // preventDefault() is your  homework if you don't know what it is
        // also look up preventPropagation both are usefull
        e.preventDefault();
        // create a new form and append it to the collectionHolder
 ...
 * creates a new form and appends it to the collectionHolder
 */
function addNewFormp() {
    var prototype = $collectionHolderp.data('prototype');
    var index = $collectionHolderp.data('index')+1;
    var newForm = prototype;
    newForm = newForm.replace(/__name__/g, index);
    ...
    $panel.append($panelBody);
    // append the removebutton to the new panel
    addRemoveButtonp($panel);
    // append the panel to the addNewItem
    // we are doing it this way to that the link is always at the bottom of the collectionHolder
    $addNewItemp.before($panel);
}
 ...
 * @param $panel
 */
function addRemoveButtonp ($panel) {
    // create remove button
    var $removeButton = $('<a href="#" class="btn btn-danger">Remove</a>');
    // appending the removebutton to the panel footer
    var $panelFooter = $('<div class="panel-footer"></div>').append($removeButton);
